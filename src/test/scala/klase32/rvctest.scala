...
class RVCTest extends KlasTest {
  behavior of "RVC"

  it should "correctly handle all cases for RVI and RVC combinations" in {
    implicit val p: Parameters = new DefaultConfig() // Ensure this matches your configuration class

    test(new RVC) { dut =>
      // Function to perform instruction fetch
      val rvcInst: BigInt = 0x1
      val rviInst: BigInt = 0x3
      val rvi = rviInst
      val rvcrvc = rvcInst << 16 | rvcInst
      val rvirvc = rviInst << 16 | rvcInst
      val rvcrvi = rvcInst << 16 | rvcInst // don't care for rviInst
      val rvirvi = rviInst << 16 | rviInst // don't care for rviInst

      def performFetch(pc: BigInt, data: BigInt, xcpt: HeartXcpt): Unit = {
        dut.io.in.valid.poke(true.B)
        dut.io.in.bits.data.poke(data)
        dut.io.in.bits.xcpt.poke(xcpt)
        dut.io.pcIs4ByteAligned.poke(pc & 0x2)
      }

      def checkOut(data: BigInt, xcpt: HeartXcpt) = {
        dut.io.out.valid.expect(true.B)
        dut.io.out.bits.data.expect(data)
        dut.io.out.bits.xcpt.expect(xcpt)
      }
